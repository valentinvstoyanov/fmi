...
void load_wallet(std::ifstream& file, Wallet& wallet) {
...
long int search_wallet_pos_at_file(std::ifstream& file, const unsigned id) {
    if(!file) {
...
bool search_wallet_file(const unsigned id, Wallet& wallet) {
    std::ifstream file;
    file.open(WALLET_FILENAME, std::ios::binary | std::ios::in);
    long int pos = search_wallet_pos_at_file(file, id);
    if(pos >= 0) {
